...
export const analyzeContent = (config: ConfigMerged): ContentInfo => {
  return {
    selection: {
      isEntireCodebase:
        !config.include.length && !config.ignore.customPatterns.length,
...
export const generateHeader = (
  ...
  if (info.processing.parsableStyle) {
    processingNotes.push(
      `content has been formatted for parsing in ${config.output.style} style`,
    );
  }
  ...
  const processingInfo =
    processingNotes.length > 0
      ? `The content has been processed where ${processingNotes.join(', ')}.`
      : '';
...
export const generateSummaryNotes = (config: ConfigMerged): string => {
  const info = analyzeContent(config);
  const notes = [
  ...
  if (info.selection.include) {
    notes.push(
      `- Only files matching these patterns are included: ${config.include.join(', ')}`,
    );
  }
  if (info.selection.ignore) {
    notes.push(
      `- Files matching these patterns are excluded: ${config.ignore.customPatterns.join(', ')}`,
    );
  }
